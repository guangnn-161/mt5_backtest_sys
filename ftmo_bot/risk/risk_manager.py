...
class RiskManager:
    ...
    def calculate_position_size(self, current_equity: float, entry_price: float,
                                stop_loss: float, contract_size: float = 100) -> float:
        if entry_price == stop_loss:
            return 0.0

        risk_amount_usd = current_equity * (self.risk_per_trade_pct / 100)
        sl_distance = abs(entry_price - stop_loss)
        raw_lot = risk_amount_usd / (contract_size * sl_distance)
    ...
    def evaluate(self, account_state: dict, signal: dict, compliance_guard) -> dict:
        ...
        'daily_start_equity', 'peak_equity' — không chỉ 'equity' như bản cũ.
        ...
        allowed, reason = compliance_guard.evaluate_entry(
            ...
            daily_start_equity=account_state['daily_start_equity'],
